...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    const result = await db.query(
      'INSERT INTO brands (name, logo, show_on_page) VALUES ($1, $2, $3) RETURNING id',
      [name, logo, show_on_page !== false ? 1 : 0]
    );
...
router.put('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    const db = getDb();
    const result = await db.query(
      'UPDATE brands SET name=$1, logo=$2, show_on_page=$3 WHERE id=$4',
      [name, logo, show_on_page !== false ? 1 : 0, req.params.id]
    );
